...
description: "Why multi-tenant AI cost control is an isolation problem — and how to enforce per-tenant budgets, quotas, and hierarchical limits so one customer never affects another."
blog: true
sidebar: false
---
...
The structural problem is clear: provider caps protect the provider's exposure to you, not your exposure to individual customers. For multi-tenant AI platforms, the enforcement boundary must exist **per customer, inside your runtime**.
...
Tenant-level budgets solve the isolation problem. But within a tenant, you still need to control which workflows, users, and individual runs can spend how much. This is where hierarchical scoping comes in.

```
Tenant: Acme Corp ($2,000/month)
├── Workflow: document-analysis ($800/month)
│   ├── User: alice ($200/month)
│   │   └── Run: run-7a3f ($25/run)
│   └── User: bob ($200/month)
│       └── Run: run-9c1e ($25/run)
├── Workflow: chat-assistant ($500/month)
│   └── User: carol ($100/month)
│       └── Run: session-4d2b ($5/session)
└── Workflow: code-review ($400/month)
    └── Run: run-2e8f ($10/run)
```

Each level in the hierarchy is a [budget scope](/protocol/how-scope-derivation-works-in-cycles). When an agent makes a reservation, the system checks budget availability **at every level in the path**: the run, the user, the workflow, and the tenant. All must have sufficient budget for the reservation to succeed. If any scope is exhausted, the request is denied.

This means:

- A single run cannot exceed its $25 ceiling, even if the tenant has $1,500 remaining
- A user cannot exceed their $200 monthly allocation, even if the workflow has capacity
...
Scopes compose naturally. You do not need to implement enforcement at every level on day one. Start with tenant budgets for isolation, then add run-level budgets for execution safety, then layer in workflow and user budgets as your product matures. The [modeling guide](/how-to/how-to-model-tenant-workflow-and-run-budgets-in-cycles) covers this progression in detail.
...
**Budgets** answer: "How much economic exposure is this tenant allowed to create?" They are real-time enforceable balances — each reservation decrements the balance atomically, and the balance reflects cumulative spend across all workflows, users, and runs.
...
**SaaS copilot with per-account monthly limits.** Each customer account gets a monthly budget tied to their subscription tier. A Starter account gets $50/month, Pro gets $500, Enterprise gets a custom amount. Each copilot session runs within a per-session budget ($2 for Starter, $10 for Pro) nested inside the monthly account budget. When a session exhausts its limit, the copilot degrades to shorter responses rather than stopping entirely. When the monthly limit hits, the customer sees a usage notice and can upgrade.
...
**Enterprise customer with departmental sub-budgets.** A large enterprise tenant gets $10,000/month. Their IT team allocates sub-budgets by department: Engineering gets $5,000, Marketing gets $2,000, Support gets $3,000. Each department's agents draw from their own scope. When Marketing's budget is exhausted mid-month, Engineering and Support are unaffected. The enterprise admin can reallocate budget between departments via the [admin API](/how-to/budget-allocation-and-management-in-cycles) without involving the platform operator.
...
4. **Layer in workflow and user budgets.** As your product matures, add scopes that match your product model — per-workflow caps for different features, per-user caps for seat-based pricing, per-department budgets for enterprise customers.
...
- **[Budget Patterns: A Practical Guide](/blog/agent-budget-patterns-visual-guide)** — six common patterns with code examples and trade-offs
