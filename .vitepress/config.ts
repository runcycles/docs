...
// Resolve the OG/Twitter image URL for a blog post. Order:
//   1. frontmatter.image (explicit per-post override)
//   2. /og/blog/<slug>.png if it exists on disk (auto-generated by scripts/generate-og-images.mjs)
//   3. site-wide fallback
function resolveBlogOgImage(pageData) {
  ...
  const m = /^blog\/([^/]+)\.md$/.exec(rel)
  if (m) {
    const slug = m[1]
    const generatedPath = path.join(srcRoot, 'public', 'og', 'blog', `${slug}.png`)
    if (fs.existsSync(generatedPath)) {
      return `https://runcycles.io/og/blog/${slug}.png`
    }
  }
...
export default defineConfig({
  ...
  transformPageData(pageData) {
    ...
    if (pageData.frontmatter.blog) {
      const ogImage = resolveBlogOgImage(pageData)

      /* Blog-specific: override og:type, add article metadata, override image if set.
         twitter:image is also pushed here so X uses the post-specific image instead
         of the global twitter:image declared in the head array. */
      pageData.frontmatter.head.push(
